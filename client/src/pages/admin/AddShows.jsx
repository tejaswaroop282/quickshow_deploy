...
const AddShows = () => {
    ...
    const [fetchError, setFetchError] = useState(null)
     ...
     const fetchNowPlayingMovies = async () => {
        try {
            ...
            if(!token){
                setFetchError('Sign in required to fetch movies')
                toast.error('Sign in required to fetch movies')
                return
            }
            ...
            if(data.success){
                ...
                const message = data.message || 'Failed to fetch movies'
                setFetchError(message)
                toast.error(message)
            }
        } catch (error) {
            console.error('Error fetching movies:', error)
            const message = error.response?.data?.message || 'Failed to fetch movies from TMDB'
            setFetchError(message)
            toast.error(message)
        } finally {
  ...
  if(fetchError) return (
    <div className='flex flex-col items-center justify-center gap-4 min-h-[50vh]'>
      ...
      <p className='text-gray-400 text-center max-w-md'>{fetchError}</p>
      <button onClick={fetchNowPlayingMovies} className='bg-primary text-white px-6 py-2 rounded hover:bg-primary/90 transition cursor-pointer'>
        Retry
      </button>
    </div>
  )
  ...
  return nowPlayingMovies.length > 0 ? (
    ...
    <div className='flex flex-col items-center justify-center gap-4 min-h-[50vh]'>
      <Title text1="Add" text2="Shows" />
      <p className='text-gray-400'>No movies found from TMDB.</p>
      <button onClick={fetchNowPlayingMovies} className='bg-primary text-white px-6 py-2 rounded hover:bg-primary/90 transition cursor-pointer'>
        Refresh
      </button>
    </div>
